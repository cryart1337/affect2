...
class pingonjoin(commands.Cog):
    ...
    async def pingonjoin(self, ctx):
        if ctx.invoked_subcommand is None:
            e = discord.Embed(title="Command: pingonjoin", description="ping new members when they join your server",color=Colours.standard, timestamp=ctx.message.created_at)
    ...
    async def add(self, ctx: commands.Context, *, channel: discord.TextChannel): 
        ...
        try:
            ...
            embed = discord.Embed(description = f"> {Emotes.approve} {ctx.author.mention}: successfully added ping new members on {channel.mention}", color = Colours.standard)
    ...
    async def clear(self, ctx):
        ...
        try:
            ...
            embed = discord.Embed(description = f"> {Emotes.approve} {ctx.author.mention}: successfully cleared ping on join", color = Colours.standard)
            await sendmsg(self, ctx, None, embed, None, None, None, None)
            await self.bot.db.commit()
        except Exception as e:
            print(e)
