...
class goodbye(commands.Cog):
    ...
    async def goodbye(self, ctx):
        e = discord.Embed(title="Command: goodbye", description="sends a message when a user leaves your guild",color=Colours.standard, timestamp=ctx.message.created_at)
    ...
    async def message(self, ctx, *, code=None):
        ...
        embed=discord.Embed(description=f"""> {Emotes.approve} {ctx.author.mention}: set goodbye message: 
    ...
    async def channel(self, ctx, channel: discord.TextChannel=None):
        if not ctx.author.guild_permissions.manage_guild: return await noperms(self, ctx, "manage_guild")  
        embed=discord.Embed(description=f"> {Emotes.approve} {ctx.author.mention}: set goodbye channel to {channel.mention}", color=Colours.standard)
        async with self.bot.db.cursor() as cursor:
            await cursor.execute("SELECT * FROM goodbye WHERE guild = {}".format(ctx.guild.id))
            check = await cursor.fetchone()
            if check is None:
                async with self.bot.db.cursor() as cursor:
                    await cursor.execute("INSERT INTO goodbye VALUES (?, ?, ?)", (ctx.guild.id, None, channel.id))
                await self.bot.db.commit()
                await sendmsg(self, ctx, None, embed, None, None, None, None)
            elif check is not None:
                async with self.bot.db.cursor() as cursor:
    ...
    async def config(self, ctx):   
      ...
      async with self.bot.db.cursor() as cursor:
        ...
        embed = discord.Embed(title="goodbye message config", color=Colours.standard)
    ...
    async def variables(self, ctx):   
      e = discord.Embed(title="Command: goodbye variables", description="here is a list of variables used to send a message when a user leaves your guild",color=Colours.standard, timestamp=ctx.message.created_at)
      e.add_field(name="category", value="config")
    ...
    async def delete(self, ctx):   
      ...
      await self.bot.db.commit()
      embed=discord.Embed(description=f"> {Emotes.approve} {ctx.author.mention}: deleted the goodbye config for *{ctx.guild.name}*", color=Colours.standard)
      await sendmsg(self, ctx, None, embed, None, None, None, None)
      ...
      async with self.bot.db.cursor() as cursor:
        ...
        if check is not None:
          ...
          try: 
            ...
            await channel.send(content=x[0],embed=x[1], view=x[2])
          except: await msg.channel.send(EmbedBuilder.embed_replacement(ctx.author, z))
        elif check is None:
          embed=discord.Embed(description=f"> {Emotes.warning} {ctx.author.mention}: goodbye message isnt configured for *{ctx.guild.name}*", color=Colours.standard)
          await sendmsg(self, ctx, None, embed, None, None, None, None)
