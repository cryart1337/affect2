    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if view.current_player == view.X:
            if interaction.user != player1:
                embed = discord.Embed(
                    color=Colours.standard, description=f"> {Emotes.warning} {interaction.user.mention}: it is not your turn yet")
...
class tictactoe(commands.Cog):
    ...
    async def tictactoe(self, ctx, member: discord.Member = None):
        if member == None:
            embed = discord.Embed(
                color=Colours.standard, description=f"> {Emotes.warning} please mention a user")
        ...
        await ctx.reply(f"{member.mention} turn", view=TicTacToe())
